...
public interface ProductRepository extends JpaRepository<Product, Long>,
    ...
    List<Product> findByPriceFromTo(@Param("fromPrice") String fromPrice, @Param("toPrice") String toPrice, Pageable pageable);

    @Query("SELECT p FROM Product p JOIN p.animals a WHERE a.id = :animalId and p.price BETWEEN :fromPrice and :toPrice" )
    ...
    @Query("SELECT p FROM Product p JOIN p.categories c WHERE c.id = :categoryId and p.price BETWEEN :fromPrice and :toPrice" )
    ...
    @Query("select count(p) from Product p where lower(p.brand) like lower(concat('%', :brand, '%') )")
    ...
    Long countPriceAllProducts(@Param("fromPrice") String fromPrice, @Param("toPrice") String toPrice);

    @Query("SELECT count(p) FROM Product p JOIN p.animals a WHERE a.id = :animalId and p.price BETWEEN :fromPrice and :toPrice" )
    Long countByAnimalAndPrice(@Param("animalId") Long animalId,
                               @Param("fromPrice") String fromPrice,
                               @Param("toPrice") String toPrice);

    @Query("SELECT count(p) FROM Product p JOIN p.categories c WHERE c.id = :categoryId and p.price BETWEEN :fromPrice and :toPrice" )
